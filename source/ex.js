...
socketServer.on('connection', function(socket){
   ...
   socket.on('newUser', function(msg){
	...
	if (match==false) {
    	...
    	socketServer.to(socket.id).emit('clearToken',UserClientMap(socket.id));

	    if(UserCount==1){

	    	socketServer.to(Clients[socket.id]).emit('setToken',UserClientMap(socket.id));
	    	var id1=setInterval(function() { 
	    		...
	    		if (UserCount==0) {
	    			...
	    			var temp=tokenScheduler();
	    			console.log("Sent Token to "+UserNames[temp]); 
	    			socketServer.emit('clearToken',"clear");
	    			socketServer.to(Clients[temp]).emit('setToken',UserNames[x]);
	    			socketServer.emit('activeUser',UserNames[temp]);
	    		}
...
function tokenScheduler(){
	if(previousTokerUser>=UserCount-1){
		previousTokerUser=0;
		//return UserNames[0];
		return previousTokerUser; //for Debugging
	}
	else{
		previousTokerUser+=1;
		//return UserNames[previousTokerUser];
		return previousTokerUser; //for debugging
	}
}
